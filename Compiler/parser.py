# coding=utf-8
import scanner
import Pascal_Helper_Files.symbol_table as symbol_tables
...
class Parser(object):
    ...
    def case_statement(self):
        ...
        e1 = self.e()
        if e1 == scanner.TOKEN_DATA_TYPE_REAL:
            raise Exception('Real type not allowed for case: ' + e1)
        self.match(scanner.TOKEN_OPERATOR_RIGHT_PAREN)
        self.match('TK_OF')
        hole_list = []
        while (self.current_token.type_of == scanner.TOKEN_DATA_TYPE_INT or
                       self.current_token.type_of == scanner.TOKEN_DATA_TYPE_CHAR or
                       self.current_token.type_of == scanner.TOKEN_CHARACTER or
                       self.current_token.type_of == scanner.TOKEN_DATA_TYPE_BOOL):
            e2 = self.e()
            self.emit(scanner.TOKEN_OPERATOR_EQUALITY, e1, e2)
            self.match(scanner.TOKEN_OPERATOR_COLON)
    ...
    def access_array(self, symbol):
        ...
        if curr_symbol.data_type == scanner.TOKEN_DATA_TYPE_INT:
            ...
            raise Exception('Array access with type %s not supported.' % curr_symbol.data_type)
